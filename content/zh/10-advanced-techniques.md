...
前面的章节让你能用好 Claude Code。这章的技巧会改变你的工作方式：自定义命令、Hooks、MCP、并行任务——每一个单独拿出来都值得花时间配置。
...
这些技巧是叠加的：自定义命令让单次操作更高效，Hooks 让重复动作自动化，MCP 打破了 Claude 的访问边界，并行任务让你同时推进多条线。每加一层，工作方式就变一次。
