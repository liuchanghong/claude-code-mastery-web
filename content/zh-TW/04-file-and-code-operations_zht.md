...
> Claude Code 可以讀任何檔案、寫任何檔案、在整個 repo 裡搜尋。
> 大多數人只用到「幫我修這段 code」。
> 這章是剩下的 80%。
...
- 讓 Claude 自主探索，給目標而不是步驟
- 精準指定修改位置（檔名 + 函數名 + 行號）
- 多檔案協同修改是 Claude Code 最大的效率優勢
- 批量操作（生成測試、文件、樣板）省去大量重複工作
- 大型任務用**計畫 → 執行 → 驗證 → 繼續**的循環
